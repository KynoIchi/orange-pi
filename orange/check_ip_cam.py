...
def ping_ipcam(ip: str, timeout: int = 1000) -> int:
...
def monitor_ipcam(ip: str, rtsp_url: str, nama_keterangan: str, interval_sec: int = 60):
    API_ENDPOINT = "http://localhost:5000/api/ipcam/quality"  # ← GANTI sesuai endpoint Flask kamu
    while True:
        timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
        ping_status = ping_ipcam(ip)
        print(f"[{timestamp}] [{nama_keterangan}] Ping: {'✅ Online' if ping_status else '❌ Offline'}")
        result = {
            "blur_score": "-",
            "contrast_score": "-",
            "fog_patch_ratio": "-",
        ...
        payload = {
            ...
            "status_ping": "Online" if ping_status else "Offline",
...
camera_list = [
    ...
    {"nama_keterangan": "Pintu Masuk", "ip": "192.168.11.35", "port": "560", "username": "admin", "password": "damin1234"},  # ✅ pakai port 560
    ...
    # {"nama_keterangan": "CCTV AWR Kaliurang", "ip": "192.168.11.63", "port": "554", "username": "admin", "password": "Damin3001"},
]
